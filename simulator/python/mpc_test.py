...
h = int(time_horizon / N)    # step time
...
t = 1               # current simulation time step, t
...
vt   = x_opt[4][t]
rt   = x_opt[5][t]
s1t  = x_opt[6][t] # are the slack variables regardes as part of the current states????????
s2t  = x_opt[7][t]
s3t  = x_opt[8][t]
x_t = [xt,yt,psit,ut,vt,rt,s1t,s2t,s3t] # current state x_t
...
# Solve the NLP
sol = solver(x0=w0, lbx=lbw, ubx=ubw, lbg=lbg, ubg=ubg)
x_opt2, u_opt2 = trajectories(sol['x'])
x_opt2 = x_opt2.full() # to numpy array
u_opt2 = u_opt2.full() # to numpy array

# Plot the result
plot_trajectories(x_opt2, x_desired, u_opt2, time_horizon+t)

############################################################################
# Maybe in 2nd iteration of MPC use option 'warm_start_same_structure':'yes'?