...
%
% Lines 231-238 were added in August 2023 (bogdan.l.hansen@ntnu.no)
%
% FLYPATH (flypath.m)
%
% Displays static and animated trajectories of missiles and air targets
% Original code was last updated: 2016-09-03
%
% SYNTAX:
%
...
function flypath(varargin)
  ...
  hold on; 
     ...
     for i = 1:pStep:maxLength
        ...
        % --- draw scene ---
        for j = 1:1:objCount
           phi = obj(j).matrix(i,4);
           theta = obj(j).matrix(i,5);  
           psi = obj(j).matrix(i,6);

           DCM_body_to_inertial = Rzyx( phi, theta, psi );      % MSS

           vertices = (DCM_body_to_inertial * obj(j).v')';
        ...
        % --- display parameters ---
        % Limits are changed such that they can be defined in NED and displayed in ENU
